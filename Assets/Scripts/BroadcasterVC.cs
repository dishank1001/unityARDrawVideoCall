...
public class BroadcasterVC : PlayerViewControllerBase
{
    ...
    {
        Debug.Log("ProcessDrawing==============>Broad");
    ...
    IEnumerator CoProcessDrawing(DrawmarkModel dm)
    {
        ...
        Debug.Log("CoProcessDrawing==============>Broad");

        string json = JsonUtility.ToJson(dm);
        byte[] data = System.Text.Encoding.UTF8.GetBytes(json);
        Debug.Log(data);
        selfDrawer = GameObject.Find("SelfDrawer").GetComponent<SelfDrawer>();
        selfDrawer.buffer = data;
        selfDrawer.length = data.Length;
        selfDrawer.onSelfDrawDrawing(data, data.Length);
